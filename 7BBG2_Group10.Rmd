...
``` {bash="false"}
# Create a nanoscript for automated downlaod of the FASTQ files
nano fastq.sh 
...
STAR 
  ...
  --readFilesIn "$baseDirectory${sample}_1.fastq.gz" "$baseDirectory${sample}_2.fastq.gz" # Specifies the imput paired end FASTQ files 
...
```{r}
# Read the gene counts data into a dataframe
...
# Set the row names of the dataframe to match the sample IDs
...
### Identify genes that are diferentially expressed between control and treated samples
...
top10_genes <- label_data[order(label_data$padj), ][1:10, ]  # Sort by padj and take top 10
...
```{r}
# Connect to Ensembl (Saccharomyces cerevisiae dataset)
#ensembl <- useMart("ensembl", dataset = "scerevisiae_gene_ensembl")
#ensembl <- useMart("ensembl", dataset = "scerevisiae_gene_ensembl", host = "http://www.ensembl.org") ## this is the mirror site
...
the genes in the dataset may not be well annotated in KEGG pathways
